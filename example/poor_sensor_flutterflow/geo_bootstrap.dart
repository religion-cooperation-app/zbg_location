...
class GeoBootstrap {
  ...
  StreamSubscription? _deviceOverrideSub;
  ...
  Future<void> _startFromFirestoreInner(String regionId) async {
    ...
    // ----- 1b) Attach runtime config + device override listeners -----
    // First runtime emission initialises the engine; subsequent emissions update
    // it live. Device overrides are merged on top so a degraded-motion device
    // can use disableStopDetection during scheduled hours without changing the
    // global config for all users.
    _configSub?.cancel();
    _deviceOverrideSub?.cancel();
    final configReady = Completer<void>();
    Map<String, dynamic>? latestRuntime;
    Map<String, dynamic>? latestDeviceOverride;

    Future<void> applyLatestConfig(String reason) async {
      final runtime = latestRuntime;
      if (runtime == null) return;

      RuntimeConfig cfg;
      try {
        cfg = _buildRuntimeConfig(
          runtime,
          deviceOverride: latestDeviceOverride,
        );
      } catch (e, st) {
        FirebaseCrashlytics.instance.recordError(
          e,
          st,
          fatal: false,
          reason: 'runtime_config_parse_failed_$reason',
        );
      ...
      if (!configReady.isCompleted) {
        ...
        fut.catchError((Object e, StackTrace st) {
          FirebaseCrashlytics.instance.recordError(
            ...
            st,
            fatal: false,
            reason: 'live_config_update_failed_$reason',
          );
        });
      }
    }
    ...
    _configSub = fs.doc('appConfig/runtime').snapshots().listen(
      (snap) {
        ...
        latestRuntime = snap.data()! as Map<String, dynamic>;
        unawaited(applyLatestConfig('runtime'));
      },
      onError: (e) {
        if (!configReady.isCompleted) {
    ...
    _deviceOverrideSub =
        fs.doc('appConfig/runtime/deviceOverrides/$uid').snapshots().listen(
      (snap) {
        latestDeviceOverride =
            snap.exists ? (snap.data()! as Map<String, dynamic>) : null;
        unawaited(applyLatestConfig('device_override'));
      },
      onError: (e, st) {
        FirebaseCrashlytics.instance.recordError(
          e,
          st,
          fatal: false,
          reason: 'device_override_load_failed',
        );
      },
    );
    try {
  ...
  bool _effectiveDisableStopDetection(
    Map<dynamic, dynamic> platform,
    Map<String, dynamic>? deviceOverride,
  ) {
    final globalRequested = platform['disable_stop_detection'] == true;
    final deviceRequested =
        deviceOverride?['disable_stop_detection_override'] == true;
    final diagnosticsRecommended =
        deviceOverride?['recommended_disable_stop_detection'] == true;
    final requested =
        globalRequested || deviceRequested || diagnosticsRecommended;
    if (!requested) return false;

    final schedule = platform['active_geo_schedule'];
    if (schedule is! Map || schedule['enabled'] != true) {
      return true;
    }

    final deviceActiveHoursOnly = deviceOverride?['active_hours_only'] != false;
    final scheduleApplies = globalRequested || deviceActiveHoursOnly;
    if (!scheduleApplies) return true;

    return _isInsideActiveGeoSchedule(schedule);
  }

  bool _isInsideActiveGeoSchedule(Map<dynamic, dynamic> schedule) {
    final windows = schedule['windows'];
    if (windows is! List || windows.isEmpty) return false;

    // Uses device-local time. If study windows must stay pinned to a specific
    // region while the user travels, replace this with timezone-package logic.
    ...
    for (final rawWindow in windows) {
      ...
      if (days is List && days.isNotEmpty) {
        final allowedDays = days
            ...
            .toSet();
        if (allowedDays.isNotEmpty && !allowedDays.contains(weekday)) {
          continue;
        }
      }

      final start = _parseScheduleMinute(rawWindow['start']);
      final end = _parseScheduleMinute(rawWindow['end']);
      if (start == null || end == null) continue;
      if (start == end) return true; // Treat equal start/end as all day.

      final active = start < end
          ? minutesNow >= start && minutesNow < end
          : minutesNow >= start || minutesNow < end; // Overnight window.
  ...
  int? _parseScheduleMinute(dynamic value) {
  ...
  RuntimeConfig _buildRuntimeConfig(
    ...
    final disableStopDetection = _effectiveDisableStopDetection(
      platform,
      deviceOverride,
    );

    FirebaseCrashlytics.instance.setCustomKey(
      'disable_stop_detection_effective',
      disableStopDetection.toString(),
    );
    FirebaseCrashlytics.instance.setCustomKey(
      'disable_stop_detection_device_override',
      (deviceOverride?['disable_stop_detection_override'] == true).toString(),
    );
    final overrideReason = deviceOverride?['reason'];
    if (overrideReason is String && overrideReason.isNotEmpty) {
      FirebaseCrashlytics.instance.setCustomKey(
        'disable_stop_detection_override_reason',
        overrideReason,
      );
    }
    ...
    return RuntimeConfig(
      ...
      minimumActivityRecognitionConfidence:
          (platform['minimum_activity_recognition_confidence'] as num?)
                  ?.toInt() ??
              75,
